...
class ResourcesDictionaries:
    ...
    def get_fullname(hpo):
        ...
        synonyms = [
            [w for w in s.split(" ") if w.lower() not in [x.lower() for x in fullname.split(" ")]]
            for s in hpo["synonyms"]
    ...
    def _load(self):
        ...
        self.hpo_dict = {
            k: {"name": v["name_id"], "full_name": self.get_fullname(v), "children": v.get("children", [])}
